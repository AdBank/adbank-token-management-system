module.exports = function(app) {
  ...
  app.post('/ownerTokenBalance', function(req, res) {
    ...
    contractObj.methods
      .balanceOf(app.contract.owner_address)
  ...
  app.post('/transferTokensInternally', async function(req, res) {
    ...
    /* ID Check */
    var fromWalletId = req.body.fromWalletId;
    if(!fromWalletId.match(/^[0-9a-fA-F]{24}$/)){ // validate fromWalletId is 12-byte ObjectId value
      return res.send({ status: false, msg: 'Invalid wallet id!' });
    }
    /* ID Check End */
    ...
    if(!toWalletId.match(/^[0-9a-fA-F]{24}$/)) { // validate toWalletId is 12-byte ObjectId value
      return res.send({ status: false, msg: 'Invalid wallet id!' });
    }
    /* ID Check End */
    ...
    var amount = parseFloat(req.body.tokenAmount); // set the amount to be transfered as a floating point number

    if(amount <= 0) { // verify the amount is greater than 0
      return res.send({ status: false, msg: 'Token amount shouldn\'t be less than 0!' });
    }

    amount = parseFloat(amount.toFixed(2)); // set floating point number to 2 decimal places

    // calculate fee based
    var fee = parseFloat(amount * app.percent / 100); // Fee to Revenue Wallet
    fee = parseFloat(fee.toFixed(2)); // trim fee to two decimal places
  ...
  function checkAuth(req) {
    ...
    if(key != app.key) return 'You are not authorized!';
